...
class SoilMoistureV3Predictor:
    def __init__(self, checkpoint_path: str, device: str = None):
        self.device = device if device else ('cuda' if torch.cuda.is_available() else 'cpu')
        print(f"Using device: {self.device}")
        
        # Load model with weights_only=True for security
        self.model = RCNN_v3(image_channels=21, history_channels=2).to(self.device)
    ...
    def predict(self, current_image_path: str, previous_image_path: str, historical_label_path: str) -> np.ndarray:
        """Predict soil moisture using current image, previous image and historical label"""
        # Load and transform data
        with rasterio.open(current_image_path) as src:
            current_image = self._transform_data(src.read().astype(np.float32), self.image_scalers)
            
        with rasterio.open(previous_image_path) as src:
            previous_image = self._transform_data(src.read().astype(np.float32), self.image_scalers)
            
        with rasterio.open(historical_label_path) as src:
            historical = self._transform_data(src.read().astype(np.float32), self.label_scalers)
            
        # Convert to tensors
        with torch.no_grad():
            current_tensor = torch.FloatTensor(current_image).unsqueeze(0).to(self.device)
            previous_tensor = torch.FloatTensor(previous_image).unsqueeze(0).to(self.device)
            historical_tensor = torch.FloatTensor(historical).unsqueeze(0).to(self.device)
            
            # Get prediction
            output = self.model(current_tensor, previous_tensor, historical_tensor)
    ...
    def predict_and_evaluate(self, image_folder: str, label_folder: str, output_dir: str = "predictions_v3"):
        ...
        for location in data_organizer.get_locations():
            ...
            for sequence in tqdm(sequences, desc="Processing sequences"):
                ...
                previous_image_path, previous_label_path = sequence[-2]
...
def main():
    checkpoint_path = "/mnt/e/soil-moisture/soil-moisture/checkpoints/rcnn_v3_new_best.pth"
    predictor = SoilMoistureV3Predictor(checkpoint_path)
